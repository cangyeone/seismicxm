...
from models.EQLarge9a import EQLargeCNN as Model
...
def main(args):
    version = "03.step1"
    model_name = f"ckpt_large/largev9a.{version}.pt" #保存和加载神经网络模型权重的文件路径, ckpt_large/largev9a.03.step1.pt
    ...
    outloss = open(f"logdir/loss/large/largev9o.{version}.txt", "a") #记录训练过程中的loss
    ...
    for step in range(50000000):
        ...
        if step % 100 == 0: #每训练100步执行
            ...
            plt.savefig(f"logdir/trainfig/Large/largev9a.{version}.png") 
            plt.cla() 
            plt.clf()
            acc_time = 0 
        #if ls < 22000:break
