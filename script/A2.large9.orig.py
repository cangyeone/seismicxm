...
import torch 
from utils.forplt import LargeData2PNW
...
def main(args):
    version = "orignal.pnw.balanced.200"#46.44GB
    model_name = f"testckpt/{version}.pt" #保存和加载神经网络模型权重的文件路径
    isb = ("balanced" in version)
    print(isb)
    data_tool = LargeData2PNW(istest=False, balanced=isb)
    ...
    if isfixed:
        optim = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), 1e-2, weight_decay=0e-4)
    else:
