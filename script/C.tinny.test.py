...
from models.EQLarge9b import EQLargeCNN as Model, Loss 
...
def main(args):
    partion = "train"
    steps = "tinny"
    model_name = f"ckpt/tinny.pt" #保存和加载神经网络模型权重的文件路径
    data_tool = LargeDataTest2(partion)
    ...
    ofile = open(f"odata/v9.{partion}.{steps}.txt", "w")
    for i in range(440):
        x, d, c = data_tool.batch_data(5)
