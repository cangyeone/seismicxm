...
if __name__ == "__main__":
    usage = "test"
    with open("data/comcat_metadata_event.pkl", "rb") as f:
    ...
    if usage == "train":
        ...
        h5keys = labels[1]
    infos = []
    for dd in tqdm.tqdm(h5keys):
        #print(infos)
        etype_id, bulk, didx, pidx, sidx, dist, einfo, sinfo, ekey, skey = dd
        data = np.load(f"data/PNW-ML/{bulk}_{didx}.npy")#.T 
        data = data.astype(np.float64)
        #print(data.shape)
        wf = Waveform(xN=prep_trace(data[0, :]), xE=prep_trace(data[1, :]), xZ=prep_trace(data[2, :]), fs=100, tP=pidx/100, tS=sidx/100, delta_km=dist)
        feats = extract_features(wf)

        f36 = feats["feature_36d"]
        infos.append([feats, etype_id, ekey, skey])
    torch.save(infos, f"odata/feature.pnw.{usage}.pth")
