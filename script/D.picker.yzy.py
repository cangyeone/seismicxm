...
from config.picker_yzy import Parameter as global_parameter
...
selsta = {} 
file1 = open("data/select.loc", "r") 
for line in file1.readlines():
    sline = line.strip().split()
    skey = ".".join(sline[0:2])
    selsta[skey] = [float(sline[2]), float(sline[3]), True]
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', '--input', default="data/0521-0522foryangbi", help="输入连续波形")
    parser.add_argument(
        '-o', '--output', default="odata/seist.01", help="输出文件名")
    parser.add_argument(
        '-m', '--model', default="pickers/seist.jit", help="模型文件lppnmodel")
