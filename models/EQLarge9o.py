...
class BRNNIN(nn.Module):
    def __init__(self, nin=64, nout=16, nlayer=1) -> None:
        super().__init__() 
        self.rnn = nn.LSTM(nin, nout, nlayer, bidirectional=True) 
    ...
    def forward(self, x):
        ...
        #x = x.permute(0, 2, 1)
        x = x.permute(2, 0, 1)
        h0 = torch.zeros([2*self.nlay, B, self.nout], dtype=x.dtype, device=x.device)
        c0 = torch.zeros([2*self.nlay, B, self.nout], dtype=x.dtype, device=x.device)
...
class EQLargeCNN(nn.Module):
    def __init__(self):
        ...
        self.transformer = BRNNIN(n_feature, n_feature, 2)
