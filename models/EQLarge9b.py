...
class Encoder(nn.Module):
    def __init__(self, n_in=35, n_out=768) -> None:
        super().__init__()
        self.layers = nn.Sequential(
            ConvBNReLU(n_in, 8, 5, 2), 
            ConvBNReLU(8, 16, 5, 2), 
            ConvBNReLU(16, 32, 5, 2), 
            ConvBNReLU(32, 64, 5, 2), 
            ConvBNReLU(64, 128, 5, 2), 
            ConvBNReLU(128, n_out, 5, 2),    
        )
    def forward(self, x):
        x = self.layers(x)
        return x 
class Decoder(nn.Module):
    def __init__(self, n_in=768, n_out=37) -> None:
        super().__init__() 
        self.layers = nn.Sequential(
            ConvBNTReLU(n_in, 128, 5, 2),#2 
            ConvBNTReLU(128, 64, 5, 2), #8
            ConvBNTReLU(64, 32, 5, 2), #32
            ConvBNTReLU(32, 16, 5, 2),  #128
            ConvBNTReLU(16, 16, 5, 2),   
            ConvBNTReLU(16, 8, 5, 2), 
            nn.Conv1d(8, n_out, 5, 1, padding=2), 
            #nn.Sigmoid()
        )
    def forward(self, x):
        x = self.layers(x)
