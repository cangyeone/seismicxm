...
def main(args):
    ...
    outloss = open(f"logdir/loss/large/largev9.{version}.txt", "a") #记录训练过程中的loss
    ...
    for step in range(50000000):
        ...
        if step % 100 == 0: #每训练100步执行
            ...
            plt.savefig(f"logdir/trainfig/Large/largev9.{version}.png") 
            plt.cla() 
            plt.clf()
            acc_time = 0 
        #if ls < 22000:break
    print("done!")
#nohup python china.large9.steps.py > logdir/large9.log 2>&1 &
#3577100
import argparse
