...
def main(args):
    version = "04b"
    model_name = f"ckpt_large/largev9.05.step0.pt" #保存和加载神经网络模型权重的文件路径
    data_tool = LargeData2()
    ...
    optim = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), 1e-3, weight_decay=0e-4)
